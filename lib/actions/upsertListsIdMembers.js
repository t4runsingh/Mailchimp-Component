...
async function processAction(msg, cfg) {
  ...
  if (recordUid) {
    ...
    const subscriberHash = crypto.createHash('md5').update(recordUid.toLowerCase()).digest('hex');
    ...
    const subscriberHash = crypto.createHash('md5').update(body.email_address.toLowerCase()).digest('hex');
    console.log('subscriber_hash:', subscriberHash);
    callParams.parameters.subscriber_hash = subscriberHash;
  }
